...
void packet_process(message* msg, int flag){
        msg->last = (flag == 0) ? ntohs(msg->last) : htons(msg->last);
        msg->port = (flag == 0) ? ntohs(msg->port) : htons(msg->port);
        msg->len = (flag == 0) ? ntohs(msg->len) : htons(msg->len);
        msg->ip = (flag == 0) ? ntohl(msg->ip) : htons(msg->ip);
        msg->id = (flag == 0) ? ntohl(msg->id) : htons(msg->id);
}
